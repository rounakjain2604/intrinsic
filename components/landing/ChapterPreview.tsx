...
export default function ChapterPreview() {
  return (
    <section className="py-24 px-6 max-w-7xl mx-auto" id="chapters">
      {/* Editorial Header */}
      <motion.div
      ...
      >
        <div className="max-w-xl">
          <p className="font-[family-name:var(--font-sans)] text-xs text-[#E8694A] uppercase tracking-[0.2em] font-medium mb-4">
            Start Here
          </p>
          <h2 className="font-[family-name:var(--font-serif)] text-3xl md:text-5xl font-normal text-[#2D2A26] leading-[1.1]">
            Start with these.
            <br />
            Free forever.
          </h2>
      ...
      {/* Bento Grid */}
      <motion.div
        className="grid grid-cols-1 md:grid-cols-3 bg-[#F5F1EA] rounded-2xl border border-[#2D2A26]/10 overflow-hidden"
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true, margin: "-100px" }}
      ...
      >
        {FREE_CHAPTERS.map((chapter, i) => {
          ...
          return (
            <motion.div key={chapter.id} variants={cardVariant}>
              <Link
                href={`/chapters/${chapter.id}`}
                className={`group relative flex flex-col p-8 md:p-12 hover:bg-[#FAF8F5] transition-colors duration-300 h-full ${chapter.spanParams}`}
              >
                <div className="flex justify-between items-start mb-12">
                  <div className="p-3 bg-[#FAF8F5] border border-[#2D2A26]/8 rounded-xl transition-transform duration-300 group-hover:-translate-y-1">
                    {chapter.icon}
                  </div>
                  <Badge variant="free" />
                </div>
                ...
                <div className="mt-auto">
                  ...
                  <h3
                    className={`font-[family-name:var(--font-serif)] text-[#2D2A26] mb-4 ${
                  ...
                  <p
                    className={`font-[family-name:var(--font-sans)] text-[#6B6560] leading-relaxed mb-8 ${
      ...
      >
        <Link
          href="/chapters"
          className="font-[family-name:var(--font-sans)] text-sm font-medium text-[#E8694A] hover-underline-draw"
        >
          View all chapters →
        </Link>
      </motion.div>
